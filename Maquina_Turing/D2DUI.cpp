...
namespace D2DUI{
	...
	ScrollLayout::ScrollLayout(Orientation o): LinearLayout(o){
		hScrollBar=std::shared_ptr<ScrollBar>(new ScrollBar);
		hScrollBar->setOpacity();
		...
		vScrollBar->setOpacity();
		vScrollBar->disableVerticalScrolling();
		vScrollBar->disableHorizontalScrolling();
		vScrollBar->position=0;
		vScrollBar->portions=1;
	}
	void ScrollLayout::reorderComponents(){
		vScrollBar->setBounds(getBounds().right-20,getBounds().top,getBounds().right,getBounds().bottom-20);
		hScrollBar->setBounds(getBounds().left,getBounds().bottom-20,getBounds().right-20,getBounds().bottom);
		int top=getBounds().top+getTopPadding();
		int left=getBounds().left+getLeftPadding();
		int components=(windows.size()+layouts.size());
		if(getOrientation()==VERTICAL){
			...
			if(top>getBounds().bottom-getBottomPadding()){
				vScrollBar->enableVerticalScrolling();
				float maxYWindows=-1.f,maxYLayouts=-1.f;
				float maxY=0.f;
				if(windows.size()!=0){
					maxYWindows=windows.at(windows.size()-1).get().getBounds().bottom;
				}
				if(layouts.size()!=0){
					maxYLayouts=layouts.at(layouts.size()-1).get().getBounds().bottom;
				}
				maxY=max(maxYWindows,maxYLayouts);
				float vpHeight=maxY-(float)(getBounds().top+getTopPadding());
				float height=(float)(getBounds().bottom-getBottomPadding())-(float)(getBounds().top+getTopPadding());
				vScrollBar->portions=max(1.f,vpHeight/height);
			}
			else{
				vScrollBar->disableVerticalScrolling();
				vScrollBar->portions=1.f;
				vScrollBar->position=0.f;
			}
			int maxwidth=0,mww=0,mwl=0;
			std::sort(windows.begin(),windows.end(),[&](const std::reference_wrapper<WindowBase>a,const std::reference_wrapper<WindowBase>b){
				return a.get().getWidth()>b.get().getWidth();
			});
			std::sort(layouts.begin(),layouts.end(),[&](const std::reference_wrapper<LayoutBase>a,const std::reference_wrapper<LayoutBase>b){
				return a.get().getWidth()>b.get().getWidth();
			});
			if(windows.size()!=0)mww=windows.at(0).get().getWidth();
			if(layouts.size()!=0)mwl=layouts.at(0).get().getWidth();
			maxwidth=max(mww,mwl);
			if(left+maxwidth>getBounds().right-getRightPadding()){
				hScrollBar->enableHorizontalScrolling();
				hScrollBar->portions=(float)(left+maxwidth)/(float)((float)(getBounds().right-getRightPadding())-(float)(getBounds().left+getLeftPadding()));
			}
			else{
				hScrollBar->disableHorizontalScrolling();
				hScrollBar->portions=1.f;
				hScrollBar->position=0.f;
			}
		}//Vertical Placement
		else if(getOrientation()==HORIZONTAL){
			for(int i=0;i<components;i++){
			...
			if(left>getBounds().right-getRightPadding()){
				hScrollBar->enableHorizontalScrolling();
				hScrollBar->portions=(float)(left-(getBounds().left+getLeftPadding()))/(float)((getBounds().right-getRightPadding())-(getBounds().left+getLeftPadding()));
			}
			else{
				hScrollBar->disableHorizontalScrolling();
				hScrollBar->portions=1.f;
			...
			int maxheight=0,mhw=0,mhl=0;
			std::sort(windows.begin(),windows.end(),[&](const std::reference_wrapper<WindowBase>a,const std::reference_wrapper<WindowBase>b){
				return a.get().getHeight()>b.get().getHeight();
			});
			std::sort(layouts.begin(),layouts.end(),[&](const std::reference_wrapper<LayoutBase>a,const std::reference_wrapper<LayoutBase>b){
				return a.get().getHeight()>b.get().getHeight();
			});
			...
			maxheight=max(mhw,mhl);
			if(top+maxheight>getBounds().bottom-getBottomPadding()){
				vScrollBar->enableVerticalScrolling();
				vScrollBar->portions=(float)(top+maxheight)/(float)((getBounds().bottom-getBottomPadding())-(getBounds().top+getTopPadding()));
			}
			else{
				vScrollBar->disableVerticalScrolling();
				vScrollBar->portions=1.f;
	...
	void ScrollLayout::recreateResources(std::shared_ptr<D2DHandle>& d2d){
		reorderComponents();
		d2d->target->CreateSolidColorBrush(ColorF(fRGBA[0],fRGBA[1],fRGBA[2]),solidbrush.ReleaseAndGetAddressOf());
		solidbrush->SetOpacity(fRGBA[3]);
		if(windows.size()!=0){
		...
		if(hScrollBar->isHorizontalScrollingEnabled())hScrollBar->recreateResources(d2d);
		if(vScrollBar->isVerticalScrollingEnabled())vScrollBar->recreateResources(d2d);
	}
	void ScrollLayout::draw(std::shared_ptr<D2DHandle>& d2d){
		solidbrush->SetColor(ColorF(fRGBA[0],fRGBA[1],fRGBA[2]));
		solidbrush->SetOpacity(fRGBA[3]);
		d2d->target->FillRoundedRectangle(RoundedRect(RectF((float)getBounds().left,(float)getBounds().top,(float)getBounds().right,(float)getBounds().bottom),radius_x,radius_y),solidbrush.Get());
		d2d->target->PushAxisAlignedClip(RectF((float)getBounds().left+getLeftPadding(),(float)getBounds().top+getTopPadding(),(float)getBounds().right-getRightPadding(),(float)getBounds().bottom-getBottomPadding()),D2D1_ANTIALIAS_MODE_ALIASED);
		...
		maxY=max(maxYWindows,maxYLayouts);
		float topY=(float)(maxY-(float)(getBounds().top+getTopPadding()))*vScrollBar->position;
		float leftY=(float)(getBounds().right-getBounds().left-getRightPadding()-getLeftPadding())*hScrollBar->position;
		...
		d2d->target->PopAxisAlignedClip();
		if(hScrollBar->isHorizontalScrollingEnabled())hScrollBar->draw(d2d);
		if(vScrollBar->isVerticalScrollingEnabled())vScrollBar->draw(d2d);
	}
	...
	void TextBox::recreateResources(std::shared_ptr<D2DHandle>& d2d){
		...
		d2d->target->CreateBitmapFromWicBitmap(d2d->formatconverter.Get(),NULL,bmp_focused.ReleaseAndGetAddressOf());
		//Normal TextBox recreation
	...
	void TextBox::draw(std::shared_ptr<D2DHandle>& d2d){
		...
		textlayout->SetFontFamilyName(font,rng);
		textlayout->SetLocaleName(locale,rng);
		if(rangesFontStyle.size()!=0){
			for(int r=0;r<rangesFontStyle.size();r++){
				textlayout->SetFontStyle(rangesFontStyle.at(r).style,rangesFontStyle.at(r).rng);
			}
		}
		if(rangesColor.size()!=0){
			for(int r=0;r<rangesColor.size();r++){
				textlayout->SetDrawingEffect(rangesColor.at(r).brush.Get(),rangesColor.at(r).rng);
			}
		}
		if(isEnabled()){
			if(hasFocus()){
				...
				d2d->target->DrawBitmap(bmp_focused.Get(),RectF((float)getBounds().left,(float)getBounds().top,(float)getBounds().right,(float)getBounds().bottom),opacity,imageQuality);
				solidbrush->SetColor(ColorF(fRGBA[0],fRGBA[1],fRGBA[2],fRGBA[3]*opacity));
				d2d->target->PushAxisAlignedClip(RectF(getBounds().left+getLeftPadding(),getBounds().top+getTopPadding(),getBounds().right-getRightPadding(),getBounds().bottom-getBottomPadding()),D2D1_ANTIALIAS_MODE_ALIASED);
				if(caretRect.left>getBounds().right){
					d2d->target->SetTransform(Matrix3x2F::Translation(getBounds().right-caretRect.left,0.f));
				}
				d2d->target->DrawTextLayout(Point2F(getBounds().left+getLeftPadding(),getBounds().top+getTopPadding()),textlayout.Get(),solidbrush.Get(),D2D1_DRAW_TEXT_OPTIONS_NO_SNAP);
				d2d->target->PopAxisAlignedClip();
				if(&caretRect!=NULL){
	...
	void LogElement::setRadius(float radiusX,float radiusY){
	...
	float LogElement::getXRadius(){
		return radiusX;
	}
	float LogElement::getYRadius(){
		return radiusY;
	}
	void LogElement::recreateResources(std::shared_ptr<D2DHandle>& d2d){
		std::wstring wtext(getText());
		switch(getVerticalTextAlignment()){
		case VerticalConstants::TOP:
			d2d->textformat->SetParagraphAlignment(DWRITE_PARAGRAPH_ALIGNMENT_NEAR);
	...
	void LogElement::draw(std::shared_ptr<D2DHandle>& d2d){
		solidbrush->SetColor(ColorF(fRGBA_bg[0],fRGBA_bg[1],fRGBA_bg[2]));
		solidbrush->SetOpacity(fRGBA_bg[3]*opacity);
		d2d->target->FillRoundedRectangle(RoundedRect(RectF((float)getBounds().left,(float)getBounds().top,(float)getBounds().right,(float)getBounds().bottom),radiusX,radiusY),solidbrush.Get());
		solidbrush->SetColor(ColorF(fRGBA[0],fRGBA[1],fRGBA[2]));
		solidbrush->SetOpacity(fRGBA[3]*opacity);
		d2d->target->DrawTextLayout(Point2F(getBounds().left+getLeftPadding(),getBounds().top+getTopPadding()),textlayout.Get(),solidbrush.Get());
		if(isHovered()){
			d2d->target->DrawRoundedRectangle(RoundedRect(RectF((float)getBounds().left,(float)getBounds().top,(float)getBounds().right,(float)getBounds().bottom),radiusX,radiusY),solidbrush.Get(),3.f);
		}
	}
	...
	void ScrollBar::draw(std::shared_ptr<D2DHandle>& d2d){
		d2d->target->CreateSolidColorBrush(ColorF(0.5f,0.5f,0.5f),solidbrush.ReleaseAndGetAddressOf());
		if(isPressed()){
			solidbrush->SetColor(ColorF(0.3f,0.3f,0.3f));
			d2d->target->FillRoundedRectangle(RoundedRect(RectF((float)getBounds().left,(float)getBounds().top,(float)getBounds().right,(float)getBounds().bottom),5.f,5.f),solidbrush.Get());
			//d2d->target->DrawBitmap(bmp_pressed.Get(),RectF((float)getBounds().left,(float)getBounds().top,(float)getBounds().right,(float)getBounds().bottom),opacity,imageQuality);
		}
		else{
			solidbrush->SetColor(ColorF(0.7f,0.7f,0.7f));
			d2d->target->FillRoundedRectangle(RoundedRect(RectF((float)getBounds().left,(float)getBounds().top,(float)getBounds().right,(float)getBounds().bottom),5.f,5.f),solidbrush.Get());
			//d2d->target->DrawBitmap(bmp.Get(),RectF((float)getBounds().left,(float)getBounds().top,(float)getBounds().right,(float)getBounds().bottom),opacity,imageQuality);
		}
		float topY=(float)(getBounds().top)+((((float)(getBounds().bottom-getBounds().top)))*position);
		float height=(float)(getBounds().bottom-getBounds().top)/portions;
		solidbrush->SetColor(ColorF(0.5f,0.5f,0.5f));
		d2d->target->FillRoundedRectangle(RoundedRect(RectF((float)getBounds().left,topY,(float)getBounds().right,min((int)(topY+height),(int)getBounds().bottom)),5.f,5.f),solidbrush.Get());
